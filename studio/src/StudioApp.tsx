...
const SECTIONS: Section[] = [
  ...
  { key: 'look',     label: 'NGOAI HINH', endpoint: '/api/appearance',   dataKey: 'rows', pk: 'id',        nameField: 'name' },
  ...
  { key: 'shop',     label: 'SHOP NPC',  endpoint: '/api/shops',         dataKey: 'rows', pk: 'id',        nameField: 'name' },
  { key: 'classchange',label: 'CHUYEN LOP',endpoint: '/api/class-change',  dataKey: 'rows', pk: 'id',        nameField: 'from_class' },
  { key: 'childshop',  label: 'SHOP TRE EM',endpoint: '/api/child-shop',   dataKey: 'rows', pk: 'id',        nameField: 'name' },
  { key: 'voice',      label: 'LOI THOAI', endpoint: '/api/voice-lines',   dataKey: 'rows', pk: 'id',        nameField: 'subtitle' },
...
export default function StudioApp() {
  ...
  const [tab, setTab] = useState<'general' | 'frames' | 'vfx' | 'fxc' | 'part' | 'spine' | 'scene' | 'preview'>('general');
        ...
        <main className="flex-1 flex flex-col min-w-0 bg-surface-950">
          <div className="flex-1 flex items-center justify-center p-6 overflow-auto">
            {section.key === 'kho'
              ...
              : tab === 'part' ? <PartComposer draft={draft} setDraft={setDraft} setMsg={setMsg} />
          ...
          <div className="flex gap-1.5 px-3 h-10 items-center border-t border-white/5 bg-surface-850">
            {(['general','frames','vfx','fxc','part','spine','scene','preview'] as const).map(t => (
              <button key={t} onClick={() => setTab(t)}
                ...
                {t === 'general' ? 'General' : t === 'frames' ? 'Frame/Tach anh' : t === 'vfx' ? 'VFX/Particle' : t === 'fxc' ? 'Effect atlas' : t === 'part' ? 'Part/Trang phuc' : t === 'spine' ? 'Spine' : t === 'scene' ? 'Canh/In-game' : 'Preview'}
              </button>
            ))}
          </div>
        </main>
