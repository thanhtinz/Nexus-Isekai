...
export default function GameStorePage() {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-gray-900 to-gray-800">
      <div className="sticky top-0 z-50 bg-[#fbf7ee]/95 backdrop-blur border-b border-[#ece3d0]">
        <div className="max-w-4xl mx-auto flex">
          {TABS.map(t => (
            <button key={t.key} onClick={() => setTab(t.key)}
              className={`flex-1 py-4 flex items-center justify-center gap-2 font-bold text-sm transition-all
                ${tab === t.key ? 'text-yellow-400 border-b-2 border-yellow-400 bg-[#fffdf7]/50' : 'text-[#5b5380] hover:text-gray-200'}`}>
