    ...
    public static RefineResult refine(long charId, long itemUid) throws Exception {
        ...
        try (Connection c = DatabaseManager.getInstance().getConnection()) {
            Map<String,Object> item = SqlSafe.queryOne(c,
                "SELECT refine_level FROM character_equipment WHERE id=? AND char_id=?", itemUid, charId);
            ...
            if (rng.nextDouble() < RATE[lv]) {
                ...
                SqlSafe.update(c, "UPDATE character_equipment SET refine_level=? WHERE id=?", lv+1, itemUid);
